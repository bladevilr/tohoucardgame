...
func _build_ui():
	...
	# ━━━ 风味属性 ━━━
	_add_section_header(vbox, "风味属性")
	_add_text(vbox, "每道菜品有一项核心属性——风味，决定上菜时的基础得分。", Color(0.8, 0.8, 0.85))
	_add_attr_row(vbox, "风味", "基础得分", "菜品激活时产出的得分，是最核心的数值。通过升星、食材、关键词等途径提升。", Color(1.0, 0.42, 0.21))
	...
	_add_keyword_row(vbox, "鲜美", "同菜系≥2道时，标记右侧邻居下次激活得分×1.8。")
	...
	_add_text(vbox, "对决持续30秒，菜品按冷却时间自动激活产出风味得分。评委根据菜品风味、上菜节奏、菜系搭配等综合评分。", Color(0.8, 0.8, 0.85))
	...
	_add_text(vbox, "主菜区（中间）：基础风味+10%", Color(0.75, 0.8, 0.75))
